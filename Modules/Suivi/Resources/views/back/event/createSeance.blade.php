...
@section('slot')
    <div class="border-b-2 border-purple-100 block ">
        <h2 class="text-2xl">Nouvel événement pour la formation {{ $evenementType->nom }}</h2>
        <div class="w-full bg-white lg:ml-4 ">
            <div class="rounded p-6">
                <form action="{{ route('seance.store') }}" method="POST">
                    @csrf
                    {{-- nom --}}
                    <input type="number" name="evenement" id="evenement" hidden value="{{ $evenementType->id }}">
                    <div class="sm:flex">
                        {{-- date --}}
                    ...
                    <div class="sm:flex">
                                ...
                                <select name="etape" id="etape"
                                    ...
                                    @foreach ($etapes as $etape)
                                        <option value="{{ $etape->id }}"
                                            {{ old('etape') == $etape->id ? 'selected' : '' }}>{{ $etape->nom }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="w-full sm:w-2/4 sm:pl-5 sm:pt-0 pt-3">
                            ...
                            <div class="flex">
                                <select name="etat" id="etat" onChange={myNewFunction(this)}
